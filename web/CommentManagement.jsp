<%-- 
    ...
    Created on : Jun 24, 2023, 1:31:15 PM
    Author     : HP
--%>
    ...
    <body style="background-image: url('image/anhnen.png')">
        <jsp:include page="header.jsp"></jsp:include>
                ...
                <table class="fl-table">
                    ...
                    <tbody> 
                        <c:choose>
                            ...
                            <p style="margin-top: 50px; font-size: 32px; color: whitesmoke; position: absolute; text-shadow:black 0.1em 0.1em 0.2em">Không tìm thấy bình luận nào</p>
                        </c:when>
                        <c:otherwise>
                            ...
                            <c:forEach items="${commentList}" var="cList">
                                <tr>
                                    ...
                                    <td onclick="window.location = 'blogDetail?id=${cList.getBlogID()}'" style="cursor: pointer; color: red; text-decoration: underline">${cList.getContent()}</td>
                                    <td>${cList.getTime()}</td>
                                    <c:choose>
                                        <c:when test="${cList.getStateID() == 1}">
                                            <td><button onclick="window.location = 'commentManage?commentID=${cList.getCommentID()}&stateID=2&page=${currentPage}&mod=2'">Hide</button></td>
                                        </c:when>
                                        <c:otherwise> 
                                            <td><button onclick="window.location = 'commentManage?commentID=${cList.getCommentID()}&stateID=1&page=${currentPage}&mod=1'">Show</button></td>
                                        </c:otherwise>
                                    </c:choose>
                                </tr>
            ...
            <div>
                ...
                <div class="paging_wrap">
                    <div class="paging_previous" style="display: <c:if test="${currentPage != 1}">none</c:if>; cursor: default">
                        ...
                        <div onclick="window.location = 'commentManage?page=${currentPage-1}'" class="paging_previous hv" style="display: <c:if test="${currentPage == 1}">none</c:if>">
                            <i class="fa-solid fa-angles-left"></i>
                        </div>
                    <c:forEach begin="1" end="${totalPage}" var="i">
                        <div onclick="window.location = 'commentManage?page=${i}'" class="paging_page hv <c:if test="${currentPage==i}">pagingActive</c:if>">${i}</div>
                    </c:forEach>
                    <div onclick="window.location = 'commentManage?page=${currentPage+1}'" class="paging_next hv" style="display: <c:if test="${currentPage == totalPage}">none</c:if>">
            ...
            <script>
                        ...
                        if ('${commentStatus }' === 'Hiển thị bình luận thành công' || '${commentStatus}' === 'Ẩn bình luận thành công') {
                            activeShowToast();
                        }
                        ...
                        function myFunction(object) {
                            ...
                            window.location = 'commentManage?stateFilter=' + value + '&page=${currentPage}&mod=3';
                        }
        </script>
    </body>
</html>
